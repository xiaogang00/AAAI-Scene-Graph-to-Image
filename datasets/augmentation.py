...
class RandomRotation(object):
    ...
    def __call__(self, clip):
        ...
        if isinstance(clip[0], np.ndarray):
            rotated = [rotate(image=img, angle=angle, preserve_range=True, order=0) for img in clip]
        elif isinstance(clip[0], PIL.Image.Image):
    ...
    def __call__(self, clip):
        ...
        if isinstance(clip[0], np.ndarray):
            for img in clip:
                ...
                # changed = [rotate(image=img, angle=angle, preserve_range=True) for img in clip]
        else:
            raise TypeError('Expected numpy.ndarray or PIL.Image' +
                            'but got list of {0}'.format(type(clip[0])))
...
class RandomShift(object):
    ...
    def __call__(self, image):
        ...
        shifted = cv2.warpAffine(image, M, (image.shape[1], image.shape[0]), flags=cv2.INTER_NEAREST)
