...
# from .utils import imagenet_preprocess
try:
    from .coco_objects_dataset3 import Cropped_COCO_Dataset, object_item
except:
    from coco_objects_dataset3 import Cropped_COCO_Dataset, object_item
...
def np_free_form_mask(maxVertex, maxLength, maxBrushWidth, maxAngle, h, w):
    ...
    for i in range(numVertex):
        ...
        cv2.line(mask, (startY, startX), (nextY, nextX), 0, brushWidth)
        cv2.circle(mask, (startY, startX), brushWidth // 2, 0)
...
def generate_rect_mask(im_size, mask_size, margin=8, rand_mask=True):
    mask = np.ones((im_size[0], im_size[1])).astype(np.float32)
    if rand_mask:
        ...
        of0 = (im_size[0] - sz0) // 2
        of1 = (im_size[1] - sz1) // 2
    mask[of0:of0 + sz0, of1:of1 + sz1] = 0
...
def generate_stroke_mask(im_size, parts=10, maxVertex=20, maxLength=100, maxBrushWidth=24, maxAngle=360):
    mask = np.ones((im_size[0], im_size[1], 1), dtype=np.float32)
...
class CocoSceneGraphDataset(Dataset):
    def __init__(self, image_dir, instances_json, stuff_json=None,
        ...
        # print('aaa', max_samples, min_objects_per_image, max_objects_per_image, kwargs["object_index_mapping"])
        ...
        # print(min_objects_per_image, max_objects_per_image, 'cccc')
        new_image_ids = []
        total_objs = 0
        for image_id in self.image_ids:
    ...
    def __getitem__(self, index):
        ...
        if 'train' in self.image_dir:
            seg_dir = 'path_to_coco/train2017_seg'
        else:
            seg_dir = 'path_to_coco/val2017_seg'

        label_nc = 184
        base_name = os.path.basename(image_path).replace('.jpg', '.png')
        seg_map_name = os.path.join(seg_dir, base_name)
        seg_map = cv2.imread(seg_map_name)[:, :, 0]
        seg_map = cv2.resize(seg_map, (self.image_size[0], self.image_size[1]), interpolation=cv2.INTER_NEAREST)
        seg_map = torch.Tensor(seg_map).long()
        seg_map[seg_map == 255] = label_nc-1
        seg_map_clone = seg_map.clone().unsqueeze(dim=0).unsqueeze(dim=0)
        # 1 * 1 * H * W
        size = seg_map_clone.size()
        seg_map_clone[seg_map_clone == 255] = label_nc-1
        ...
        for obj_idx, object_data in enumerate(self.image_id_to_objects[image_id]):
            object_index = self.object_index_mapping[image_id][obj_idx] if self.object_index_mapping is not None else None
        ...
        if self.use_object_crops:
            ...
            dim_all = original_crops.shape[1]  ## original
            obj_tuple += (selected_crops[:, :, 0:dim_all - 1, :, :],)
            obj_tuple += (original_crops[:, 0:dim_all - 1, :, :],)
            ############################################################################

        if self.build_canvas:
            assert self.crop_num <= 1, "Please disable [build canvas] in Model Options"
            canvas_sel = make_canvas_baseline(boxes, selected_crops[:, 0, 0:3, :, :], H, W)
            img_tuple += (canvas_sel, )
            canvas_ori = make_canvas_baseline(boxes, original_crops[:, 0:3, :, :], H, W)
            img_tuple += (canvas_ori, )
        img_tuple += (seg_map, )
        img_tuple += (input_label, )
