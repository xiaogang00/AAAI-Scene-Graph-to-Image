...
###########
def visualize_sample3_img(model, patch_discriminator, batch, vocab):
    (imgs, canvases_sel, canvases_ori, seg_map, label_map,
        objs, boxes, selected_crops,
        original_crops, triples, predicates,
        obj_to_img, triple_to_img,
    ...
    # add the ground-truth images
    samples.append(imgs[:1])
    patch_discriminator.to(samples[0].device)
    original_device = patch_discriminator.center_example.device
    patch_discriminator.center_example = patch_discriminator.center_example.to(samples[0].device)
    ...
    for mm in range(obj_num):
        ...
        obj_img_this2 = obj_crop_sel[mm:mm + 1, 0, 0:3, :, :]
    ...
    retrieved_patchs = []
    index_this = (obj_to_img == 0)
    selected_crops_this = selected_crops[index_this, :, 0:3, :, :]
    # b *k * c* h* w
    bbox_this_all = boxes[index_this]
    ...
    if batch_size <= 2:
        ...
        bbox_this = bbox_this_all[obj_mask]
        ...
        for kkk in range(1, selected_crops_this.shape[0], 1):
            ...
            value_list = torch.cat(value_list, dim=0).view(length, )
    ...
    retrieved_patchs = torch.cat(retrieved_patchs, dim=0)

    retrieved_patchs_all = original_crops[:, 0:3, :, :].clone()
    retrieved_patchs_all[index_this] = retrieved_patchs
    with torch.no_grad():
        model_out = model(objs, triples, obj_to_img, triple_to_img,
                          boxes_gt=boxes,
                          selected_crops=retrieved_patchs_all,
                          original_crops=original_crops,
                          scatter_size_obj=scatter_size_obj,
                          scatter_size_triple=scatter_size_triple)
    ...
    pred_bbox = model_out[0]

    retrieved_patchs = []
    index_this = (obj_to_img == 0)
    selected_crops_this = selected_crops[index_this, :, 0:3, :, :]
    # b *k * c* h* w
    bbox_this_all = pred_bbox.to(samples[0].device)
    batch_size = selected_crops_this.shape[0]
    if batch_size <= 2:
        retrieved_patchs.append(selected_crops_this[:, 0, 0:3, :, :])
    else:
    ...
    patch_discriminator.center_example = patch_discriminator.center_example.to(original_device)
    return samples



###########
def visualize_sample3_img_vg(model, patch_discriminator, batch, vocab):
    ...
    if batch_size <= 2:
        ...
        for kkk in range(1, selected_crops_this.shape[0], 1):
            ...
            value_list = []
            for kk in range(patch_this_list.shape[0]):
                patch_this = patch_this_list[kk:kk + 1]
                input_this = torch.cat([previous_patch, patch_this], dim=0)
                input_this_bbox = torch.cat([previous_bbox, bbox_this_one], dim=0)
                with torch.no_grad():
                    score = patch_discriminator(input_this, input_this_bbox)
    ...
    retrieved_patchs_all = original_crops.clone()
    retrieved_patchs_all[index_this] = retrieved_patchs
    with torch.no_grad():
        model_out = model(objs, triples, obj_to_img, triple_to_img,
                          boxes_gt=boxes,
