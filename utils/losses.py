...
def hinge_d_loss(scores_real, scores_fake):
    assert scores_real.size() == scores_fake.size()
    if scores_real.dim() > 1:
        scores_real = scores_real.view(-1)
        scores_fake = scores_fake.view(-1)

    minval = torch.min(scores_real - 1, get_zero_tensor(scores_real))
    loss_real = -torch.mean(minval)
    minval = torch.min(-scores_fake - 1, get_zero_tensor(scores_fake))
