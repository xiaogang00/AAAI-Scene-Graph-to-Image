...
def visualize_sample3_img_vg(model, batch, vocab):
    ...
    selected_crops_this = selected_crops[index_this]
    # b *k * c* h* w
    ...
    if batch_size <= 2:
        ...
        for kkk in range(1, selected_crops_this.shape[0], 1):
            ...
            for kk in range(patch_this_list.shape[0]):
                ...
                with torch.no_grad():
                    score = model(input_this, input_this_bbox)
    ...
    image_this_ori = torch.cat(retrieved_patchs, dim=0)
    layout_boxes = bbox_this_all
    H = 256
    W = 256
    input_img_ori = boxes_to_layouts(image_this_ori[:, 0:3, :, :], layout_boxes, H, W)
    input_img_ori = torch.sum(input_img_ori, dim=0).unsqueeze(dim=0)
    samples.append(input_img_ori)
