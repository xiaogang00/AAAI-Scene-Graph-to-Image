...
class VgSceneGraphDataset(Dataset):
  ...
  def __getitem__(self, index):
    ...
    objid = torch.LongTensor(O).fill_(-1)
    ...
    for i, obj_idx in enumerate(obj_idxs):
      ...
      objid[i] = self.data['object_ids'][index, obj_idx].item()
      # print(self.data['object_ids'][index, obj_idx])
    ...
    return image, objs, boxes, triples, objid


def vg_collate_fn2(batch):
  """
...
def vg_collate_fn(batch):
  ...
  """
  # batch is a list, and each element is (image, objs, boxes, triples)
  all_imgs, all_objs, all_boxes, all_triples, all_objid = [], [], [], [], []
  all_obj_to_img, all_triple_to_img = [], []
  obj_offset = 0
  for i, (img, objs, boxes, triples, objid) in enumerate(batch):
    all_imgs.append(img[None])
    O, T = objs.size(0), triples.size(0)
    all_objs.append(objs)
  ...
  all_imgs = torch.cat(all_imgs)
  all_objs = torch.cat(all_objs)
  all_objid = torch.cat(all_objid)
  all_boxes = torch.cat(all_boxes)
  all_triples = torch.cat(all_triples)
  all_obj_to_img = torch.cat(all_obj_to_img)
  all_triple_to_img = torch.cat(all_triple_to_img)

  out = (all_imgs, all_objs, all_objid, all_boxes, all_triples,
         all_obj_to_img, all_triple_to_img)
  return out
