...
class CocoSceneGraphDataset(Dataset):
  def __init__(self, image_dir, instances_json, stuff_json=None,
    ...
    stuff_data = None
    ## stuff_json = None #####!!!! delete if there is segmentation file
    ...
    # print(len(self.image_ids), 'cccc', min_objects_per_image, max_objects_per_image)
  ...
  def __getitem__(self, index):
    ...
    #####!!!! original statements if there is segmentation file
    if 'train' in image_path:
      seg_dir = '/mnt/proj3/xgxu/AttnGAN/data/coco/train2017_seg'
      # seg_dir = '/home/ubuntu/xgxu/sg2im-master/train2017'
    else:
      seg_dir = '/mnt/proj3/xgxu/AttnGAN/data/coco/val2017_seg'
      # seg_dir = '/home/ubuntu/xgxu/sg2im-master/val2017'
    label_nc = 184
    base_name = os.path.basename(image_path).replace('.jpg', '.png')
    seg_map_name = os.path.join(seg_dir, base_name)
    seg_map = cv2.imread(seg_map_name)
    # print(np.unique(seg_map), seg_map.shape)
    seg_map = cv2.resize(seg_map, (1024, 1024), interpolation=cv2.INTER_NEAREST)[:, :, 0]
    # print(np.unique(seg_map), seg_map.shape)
    # seg_map = cv2.resize(seg_map, (16, 16), cv2.INTER_NEAREST)[:, :, 0]
    # seg_map = seg_map * 1.0 / label_nc
    seg_map = torch.Tensor(seg_map)

    '''
    seg_map = np.zeros((1024, 1024))
    seg_map = torch.Tensor(seg_map).float()
    '''
    ...
    return image, objs, boxes, masks, triples, image_id, seg_map
...
def coco_collate_fn(batch):
  ...
  all_id = []
  all_seg = []
  for i, (img, objs, boxes, masks, triples, image_id, seg_mp) in enumerate(batch):
    all_imgs.append(img[None])
    # print(triples)
    # print(triples.shape, obj_offset)
  ...
  all_id = torch.Tensor(all_id)

  out = (all_imgs, all_objs, all_boxes, all_masks, all_triples,
         all_obj_to_img, all_triple_to_img, all_id, all_seg)
  return out
