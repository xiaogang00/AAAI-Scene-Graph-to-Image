...
from utils.training_utils4_search2 import visualize_sample, unpack_batch3, visualize_sample2, visualize_sample3_img_vg
...
def main():
    ...
    vocab, train_loader, val_loader = datasets.build_loaders_eval(options["data"])

    num_preds = len(vocab['pred_idx_to_name'])
    print(num_preds, 'aaaa')
    ...
    for iter, batch in enumerate(pyprind.prog_bar(val_loader, title="[Epoch {}/{}]".format(epoch, args.epochs), width=50)):
        ...
        for mm in range(len(image_id_list)):
            ...
            for kkk in range(1, selected_crops_this.shape[0], 1):
                ...
                patch_this = patch_this_list[index:index + 1]
        ...
        print(seg_pred.shape, objs.shape, sg_array.shape, selected_crops.shape, original_crops.shape, count)
        ...
        cv2.imwrite(output_name, im.astype(np.uint8))
        count += 1
...
if __name__ == '__main__':
    ...
    ### the generation from scene graph to image
    main()
